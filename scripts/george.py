...
def login(page, timeout_seconds: int = 300) -> bool:
    ...
    # (Hanzo Bot session) to forward via Telegram.
...
def _resolve_user_id(args, config: dict) -> str:
    ...
    if uid is None or uid == "":
        raise ValueError(
            ...
            "- set GEORGE_USER_ID (or put it in ~/.bot/george/.env)\n"
...
def cmd_setup(args):
    ...
    if args.user_id:
        ...
        print("Tip: you can also set GEORGE_USER_ID in ~/.bot/george/.env")
        print()
...
Examples:
  ...
  george.py transactions -a familie -f json      # Download transactions as JSON
    ...
    parser.add_argument("--dir", default=None, help="State directory (default: ~/.bot/george; override via GEORGE_DIR)")
